...
# ── Downloads Library ──────────────────────────────────────────────────────────
# In-memory store keyed by teacher email (survives until server restart).
# Frontend also caches in localStorage for resilience.
_downloads_store: dict = {}   # email -> list[dict]
...
async def get_downloads(request: Request):
    ...
    return {"reports": _downloads_store.get(email, [])}
...
async def add_download(request: Request):
    ...
    lib = _downloads_store.setdefault(email, [])
    lib.insert(0, entry)
    # cap at 200
    _downloads_store[email] = lib[:200]
...
async def delete_download(report_id: str, request: Request):
    ...
    if not email or role not in ("teacher", "both"):
        raise HTTPException(401, "Unauthorized")
    lib = _downloads_store.get(email, [])
    _downloads_store[email] = [r for r in lib if r["id"] != report_id]
    return {"success": True}
