...
def run_code(code: str, language: str = "python", stdin: str = None) -> RunResult:
    ...
    if lang in ("python", "python3"):
        ...
        gpp_exe = find_executable("g++")
        if not gpp_exe:
            return RunResult("C++ compiler error: 'g++' compiler not found on the server. Please install GCC.", error=True)

        fd, cpp_path = tempfile.mkstemp(suffix=".cpp")
        os.close(fd)
        exe_path = cpp_path.replace(".cpp", ".exe") if os.name == "nt" else cpp_path.replace(".cpp", "")
        ...
        if not gcc_exe:
            return RunResult("C compiler error: 'gcc' compiler not found on the server. Please install GCC.", error=True)
