...
def serve_frontend():
    ...
    # FIX: Read file fresh on every request so Content-Length always matches
    # the actual file size. FileResponse can cache a stale size on Windows with
    # --reload, causing "Response content longer than Content-Length" crashes.
...
def list_students(code: str):
    """Return roster of active and waiting students."""
    s = _S(code)
    ...
    try:
        while True:
            ...
            # ── VC SIGNALING RELAY ──────────────────────────────────────
                ...
                # Teacher started VC — broadcast invite to all students
                s["vc_active"] = True
                s["vc_teacher"] = ws
                vc = s.setdefault("video_call", {})
                vc["active"] = True
                vc["started_at"] = now()
                vc["screen_share_active"] = False
                vc.setdefault("participants", {})
                await ws_all_students(s, {
                    "type": "vc_started",
                    "session_code": session_code,
                    "teacher_name": s.get("teacher_name", "Teacher"),
                })
                log.info("[VC] Teacher started VC in session %s", session_code)
                ...
                vc["screen_share_active"] = False
                vc["participants"] = {}
                await ws_all_students(s, {"type": "vc_ended", "session_code": session_code})
                log.info("[VC] Teacher ended VC in session %s", session_code)

            elif cmd == "vc_sdp":
                # Teacher sending SDP to a specific student
                to_id = data.get("to")
                if to_id:
                    await ws_student(s, to_id, {**data, "from": "teacher"})

            elif cmd == "vc_ice":
                # Teacher sending ICE candidate to a specific student
                to_id = data.get("to")
                if to_id:
                    await ws_student(s, to_id, {**data, "from": "teacher"})

            elif cmd == "vc_mute_update":
                # Teacher mute state — broadcast to students
                await ws_all_students(s, {**data, "from": "teacher"})

            elif cmd == "vc_screen_share_start":
                # Teacher started screen sharing — notify all students
                vc = s.setdefault("video_call", {})
                vc["screen_share_active"] = True
                await ws_all_students(s, {
                    "type": "vc_screen_share_start",
                    "session_code": session_code,
                })
                log.info("[VC] Teacher started screen share in session %s", session_code)

            elif cmd == "vc_screen_share_stop":
                # Teacher stopped screen sharing — notify all students
                vc = s.setdefault("video_call", {})
                vc["screen_share_active"] = False
                await ws_all_students(s, {
                    "type": "vc_screen_share_stop",
                    "session_code": session_code,
                })
                log.info("[VC] Teacher stopped screen share in session %s", session_code)
...
async def student_ws_endpoint(ws: WebSocket, session_code: str, student_id: str):
    ...
    await ws_send(ws, {
        ...
        "vc_teacher_name":     s.get("teacher_name", "Teacher"),
    })
    ...
    try:
        while True:
            ...
            # ── VC SIGNALING RELAY (student side) ──────────────────────
            elif cmd == "vc_join":
                # Student joining VC — relay to teacher and track participant
                student_name = s["students"].get(student_id, {}).get("name", student_id)
                vc = s.setdefault("video_call", {})
                vc.setdefault("participants", {})[student_id] = {
                    "name": student_name,
                    "joined_at": now(),
                    "mic_on": True,
                    "cam_on": True,
                ...
                log.info("[VC] Student %s joined VC in session %s", student_id, session_code)

            elif cmd == "vc_leave":
                # Student leaving VC — relay to teacher and remove from participants
                vc = s.setdefault("video_call", {})
                vc.get("participants", {}).pop(student_id, None)
                await ws_teacher(s, {**data, "from": student_id})
                log.info("[VC] Student %s left VC in session %s", student_id, session_code)

            elif cmd == "vc_sdp":
                # Student SDP — relay to teacher (to = "teacher")
                await ws_teacher(s, {**data, "from": student_id})

            elif cmd == "vc_ice":
                # Student ICE candidate — relay to teacher
                await ws_teacher(s, {**data, "from": student_id})

            elif cmd == "vc_mute_update":
                # Student mute status — update tracking and relay to teacher
                vc = s.setdefault("video_call", {})
                part = vc.get("participants", {}).get(student_id)
                if part:
                    part["mic_on"] = bool(data.get("micOn", True))
                await ws_teacher(s, {**data, "from": student_id})
