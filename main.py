...
@app.get("/api/test/{session_code}/state")
def get_test_state(session_code: str):
    """Return full test state including remaining time. Used by frontend on reconnect/remount."""
    s = _S(session_code)
    ts = s["test_state"]
    if not ts.get("active"):
        return {"active": False}
    elapsed = time.time() - ts["start_time"] if ts.get("start_time") else 0
    remaining = max(0, ts["duration_secs"] - elapsed)
    test_tasks = [safe_task(t) for t in s["tasks"] if t["id"] in ts.get("task_ids", [])]
    return {
        "active":          True,
        ...
        "submitted_count": len(ts.get("submitted", set())),
    }
...
async def teacher_ws_endpoint(ws: WebSocket, session_code: str):
    ...
    # Build persistent test state for reconnect recovery
    ts = s["test_state"]
    test_state_payload = None
    if ts.get("active") and ts.get("start_time"):
        elapsed = time.time() - ts["start_time"]
        remaining = max(0, ts["duration_secs"] - elapsed)
        test_tasks = [safe_task(t) for t in s["tasks"] if t["id"] in ts.get("task_ids", [])]
        test_state_payload = {
...
async def student_ws_endpoint(ws: WebSocket, session_code: str, student_id: str):
    ...
    # Build persistent test state for student reconnect recovery
    ts_s = s["test_state"]
    student_test_state = None
    if ts_s.get("active") and ts_s.get("start_time"):
        elapsed_s = time.time() - ts_s["start_time"]
        remaining_s = max(0, ts_s["duration_secs"] - elapsed_s)
        test_tasks_pool = [safe_task(t) for t in s["tasks"] if t["id"] in ts_s.get("task_ids", [])]
        student_test_state = {
            "active":         True,
            "remaining_secs": int(remaining_s),
            "duration_secs":  ts_s["duration_secs"],
            "start_time":     ts_s["start_time"],
            "tasks":          test_tasks_pool,
        }
